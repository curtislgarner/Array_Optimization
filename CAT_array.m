...
Number_of_mics=[8];
Angle_ranges=[0*pi/4 5*pi/4];

fRange=500:2000;

fWeights=(2-(800-fRange).^4/3200^4-(2400-fRange).^8/1900^8)/2;
deg=Angle_ranges(1,1):pi/180:Angle_ranges(1,2);
aWeights=1-(1.96-deg).^4/16;

N_start=10;
Range=[-0.3 0.3; -0.15 0.2];
...
%         fWeights=ones(1,length(fRange));
        ...
        for i=1:N_start
            ...
            cOpt=cOpt+1;
            Cstart(i)=array_criterion(p,doa,fRange,fWeights,aWeights);
            %Text output
        ...
        for i=1:sum(C<=min(C)*1.05) % Loop through equivalent solutions (i) 
            d_max(i)=0;
...
% sol.locations=[0 0; 0 0.075; 0 0.125];
        ...
        title(num2str(total_reduction(array.FRF)));
        subplot(1,5,1:2)
